...
BayIS_All <- lapply(2:44, function(x)
...
OOS_Bayes_Detected <- lapply(2:43, function(x)
  left_join(
...
# The returns for the list above throughout each month
OOS_Bayes_Detected_Rtns <- lapply(2:43, function(x) 
  left_join(
    BayProbIS %>% dplyr::select(Funds, x) %>% 
      filter(!is.na(.[[2]]) & .[[2]] >= 0.95) %>% 
      dplyr::select(Funds),
...
# One has a list per period with the detected funds in the 
# 5-year period, next period and t + 1 period IS
OLSIS_All <-lapply(2:44, function(x)
...
IS_OLS_Detected <- lapply(2:44, function(x) 
  left_join(
...
# The returns for the list above throughout each month
IS_OLS_Detected_Rtns <- lapply(2:44, function(x) 
  left_join(
    IS_OLS_Detected[[x-1]] %>% dplyr::select(Funds),
    RealReturns  %>% dplyr::select(Funds, returnrf, date, mtna, mnav)%>%
      filter(date >= '1979-12-01'), 
    by = 'Funds')
)
...
# One has a list per period with the detected funds in the 
# 5-year period, next period and t + 1 period OOS
OOS_OLS_Detected <- lapply(2:43, function(x) 
  inner_join(
...
# The returns for the list above throughout each month
OOS_OLS_Detected_Rtns <- lapply(2:43, function(x) 
  left_join(
    OOS_OLS_Detected[[x-1]] %>% dplyr::select(Funds),
    RealReturns  %>% dplyr::select(Funds, returnrf, date, mtna, mnav) %>%
      filter(date >= '1979-12-01'), 
    by = 'Funds')
)
