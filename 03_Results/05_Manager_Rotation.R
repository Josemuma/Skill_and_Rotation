...
manager_list_positive <- lapply(19:41, function(x)
  lapply(1:nrow(list_detected_positive[[x]]), function(i) 
    wrds_hdrHist %>% filter(Funds == list_detected_positive[[x]][[1]][i]) %>% 
...
manager_list_positive # 23 lists
...
Bay_Mgr_Table <- readRDS("Bay_Mgr_Table") #####
...
# m = length(manager_list_positive) # Number of periods starting in 1993-1997
# Bay_Mgr_Table <- lapply(19:(19 + m -1), function(t)
#   tibble(bind_cols(
#     Funds = sapply(1:nrow(list_detected_positive[[t]]), function(i)
#       list_detected_positive[[t]][[1]][[i]]),
#     
#     IS_Period = sapply(1:nrow(list_detected_positive[[t]]), function(i)
#       nrow(wrds_hdrHist %>%
#              dplyr::select(Funds,mgr_name,mgr_dt,chgdt,chgenddt,adv_name) %>%
#              filter(Funds == list_detected_positive[[t]][[1]][[i]] &
#                       chgdt >= paste(1974+t,'-01-01',sep = "") &
#                       chgdt <= paste(1978+t,'-11-30',sep = "")))),
#     
#     OOS = sapply(1:nrow(list_detected_positive[[t]]), function(i)
#       nrow( wrds_hdrHist %>%
#               dplyr::select(Funds,mgr_name,mgr_dt,chgdt,chgenddt,adv_name) %>%
#               filter(Funds == list_detected_positive[[t]][[1]][[i]] &
...
#     
#     OOS_Plus1 = sapply(1:nrow(list_detected_positive[[t]]), function(i)
#       nrow( wrds_hdrHist %>%
#               dplyr::select(Funds,mgr_name,mgr_dt,chgdt,chgenddt,adv_name) %>%
#               filter(Funds == list_detected_positive[[t]][[1]][[i]] &
...
#   )))
# saveRDS(Bay_Mgr_Table,file = "Bay_Mgr_Table") ####
...
## Manager Tot Changes ####
manager_total_changes_p <- sapply(1:(length(Bay_Mgr_Table)), function(x)
  colSums(Bay_Mgr_Table[[x]][,2:4]))
# Transpose
manager_total_changes_p <- as_tibble(cbind(nms = names(manager_total_changes_p), 
                                          t(manager_total_changes_p)))
# Add periods (years)
manager_total_changes_p <- tibble(cbind(Periods = 1998:2020,
                                         manager_total_changes_p))
manager_total_changes_p # Total of rotation per period

# Table of average rotation per period IS, OOS, OOS_P1 ####
manager_mean_p <- tibble(bind_cols(
  Periods = 1998:2020,
  t(sapply(1:(length(Bay_Mgr_Table)), function(x) 
    colMeans(Bay_Mgr_Table[[x]][,2:4])))))
#####
manager_mean_p %>% print(n = Inf) # Print all periods
colMeans(manager_mean_p[,2:4]) # Average rotations per period
...
# periods to only 22.
# We use Bay_Mgr_Table that was previously created. 
# 18 + t because that would be 1998
manager_pers_skill_p <- lapply(1:(length(Bay_Mgr_Table)), function(t)
  right_join(Bay_Mgr_Table[[t]],
             list_detected_positive[[18 + t]] %>% filter(.[[3]] >=0.95), 
             by  = 'Funds'))
manager_pers_skill_p[[6]] # Example should show 3
# Mean
colMeans(tibble(t(sapply(1:length(Bay_Mgr_Table), function(x)
  colMeans((manager_pers_skill_p[[x]])[,c(2:13,15:17,19:21)],na.rm = T)))),na.rm = T)
# Occasional skilled funds rotation ####
manager_occa_skill_p <- lapply(1:(length(Bay_Mgr_Table)), function(t)
  right_join(Bay_Mgr_Table[[t]],
             list_detected_positive[[18 + t]] %>% filter(.[[3]] < 0.95), 
             by  = 'Funds'))
manager_occa_skill_p[[6]] # Example should show 67
# Mean
colMeans(tibble(t(sapply(1:length(Bay_Mgr_Table), function(x)
  colMeans((manager_occa_skill_p[[x]])[,c(2:13,15:17,19:21)],na.rm = T)))),na.rm = T)
# Continuous persistent skill ####
manager_contin_pers_skill_p <- lapply(1:(length(Bay_Mgr_Table)), function(t)
  right_join(Bay_Mgr_Table[[t]],
             list_detected_positive[[18 + t]] %>% filter(.[[3]]>=0.95 & .[[4]] >=0.95), 
             by  = 'Funds'))
manager_contin_pers_skill_p[[6]]
# Mean
colMeans(tibble(t(sapply(1:length(Bay_Mgr_Table), function(x)
  colMeans((manager_contin_pers_skill_p[[x]])[,c(2:13,15:17,19:21)],na.rm = T)))),na.rm = T)
# Occasional persistent skill ####
manager_occas_pers_skill_p <- lapply(1:(length(Bay_Mgr_Table)), function(t) 
  right_join(Bay_Mgr_Table[[t]],
             list_detected_positive[[18 + t]] %>% filter(.[[3]] >=0.95 & .[[4]] < 0.95), 
             by  = 'Funds'))
manager_occas_pers_skill_p[[6]]
# Mean
colMeans(tibble(t(sapply(1:length(Bay_Mgr_Table), function(x)
  colMeans((manager_occas_pers_skill_p[[x]])[,c(2:13,15:17,19:21)],na.rm = T)))),na.rm = T)
# Bounce skill ####
manager_bounce_skill_p <- lapply(1:(length(Bay_Mgr_Table)), function(t) 
  right_join(Bay_Mgr_Table[[t]],
             list_detected_positive[[t + 18]] %>% filter(.[[3]] < 0.95 & .[[4]] >= 0.95), 
             by  = 'Funds'))
manager_bounce_skill_p[[6]]
# Mean
colMeans(tibble(t(sapply(1:length(Bay_Mgr_Table), function(x)
  colMeans((manager_bounce_skill_p[[x]])[,c(2:13,15:17,19:21)],na.rm = T)))),na.rm = T)
# Drop skill
manager_drop_skill_p <- lapply(1:(length(Bay_Mgr_Table)), function(t) 
  right_join(Bay_Mgr_Table[[t]],
             list_detected_positive[[t + 18]] %>% filter(.[[3]] < 0.95 & .[[4]] < 0.95), 
             by  = 'Funds'))
manager_drop_skill_p[[6]]
# Mean
colMeans(tibble(t(sapply(1:length(Bay_Mgr_Table), function(x)
  colMeans((manager_drop_skill_p[[x]])[,c(2:13,15:17,19:21)],na.rm = T)))),na.rm = T)
